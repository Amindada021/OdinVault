...
internal sealed class AgentDashboardClient : IDisposable
{
 ...
 public async Task<RestoreExecutionClientResponse?> RestoreAsync(Guid backupId,string target){using var r=Authorized(HttpMethod.Post,"api/restores");r.Content=JsonContent.Create(new RestoreClientRequest(backupId,target),options:Json);using var response=await http.SendAsync(r);response.EnsureSuccessStatusCode();return await response.Content.ReadFromJsonAsync<RestoreExecutionClientResponse>(Json);}
 ...
 public async Task TestStorageAsync(Guid id){using var r=Authorized(HttpMethod.Post,$"api/storage-targets/{id}/connection-test");using var response=await http.SendAsync(r);response.EnsureSuccessStatusCode();}
