...
internal sealed class AgentDashboardClient : IDisposable
{
 ...
 public async Task DeleteDatabaseAsync(Guid id,bool deleteHistory=true,bool deleteFiles=false){using var request=new HttpRequestMessage(HttpMethod.Delete,$"api/databases/{id}?deleteHistory={deleteHistory.ToString().ToLowerInvariant()}&deleteFiles={deleteFiles.ToString().ToLowerInvariant()}");request.Headers.TryAddWithoutValidation("X-OdinVault-Key",LoadApiKey());using var response=await http.SendAsync(request);response.EnsureSuccessStatusCode();}public async Task<IReadOnlyList<DatabaseOverviewResponse>> DatabasesAsync(){
