...
internal sealed class MainViewModel : ObservableObject, IDisposable
{
 ...
 public bool IsLoading{get=>loading;private set=>Set(ref loading,value);} public string? Error{get=>error;private set{if(Set(ref error,value))Raise(nameof(HasError));}} public bool HasError=>!string.IsNullOrWhiteSpace(Error);
